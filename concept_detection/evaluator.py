...
# IMAGECLEF 2023 CAPTION - CONCEPT DETECTION
class ConceptEvaluator:
    ...
    with open(submission_file_path) as csvfile:
        ...
        for row in reader:
          if "ID" not in row[0]:
            ...
            if not 1 <= len(row) <=2:
                self.raise_exception("Wrong format. Each line must at least consist of an image ID (no file ending), optionally followed"+
                     "by a comma (,) and 1 or more concepts separated my a semicolon ({}).", lineCnt,
  ...
  def compute_secondary_score(self, predictions):
    ...
    allowed_concepts = {"C0002978", "C0040405", "C0024485", "C0032743", "C0041618", "C1306645",
